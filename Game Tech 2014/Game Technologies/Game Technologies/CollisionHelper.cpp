...
bool CollisionHelper::PlaneBBCollision(PhysicsNode& p0, PhysicsNode& p1)
{
	CollisionPlane& plane = *(CollisionPlane*)p0.GetCollisionVolume();
	CollisionAABB& aabb = *(CollisionAABB*)p1.GetCollisionVolume();

	if (p1.GetPosition().y < -5)
	{
		int a = 1;
	}

	Vector3 dist = p0.GetPosition() - p1.GetPosition();
	Vector3 sum = plane.GetSize() + aabb.getHalfDimensions();
	if (abs(dist.x) <= sum.x && abs(dist.y) <= sum.y && abs(dist.z) <= sum.z)
	{
		return true;
	}
	else
...
void CollisionHelper::PlaneBBCollisionImpulse(PhysicsNode& p0, PhysicsNode& p1)
{
	Vector3 v = p1.GetLinearVelocity();
	v.y = 0.1f;
	p1.SetLinearVelocity(v);
}

void CollisionHelper::AABBCollisionImpulse(PhysicsNode& p0, PhysicsNode& p1)
{
	CollisionAABB& aa = *(CollisionAABB*)p0.GetCollisionVolume();
	CollisionAABB& bb = *(CollisionAABB*)p1.GetCollisionVolume();
	Vector3 dist = p1.GetPosition() - p0.GetPosition();
	Vector3 v = p1.GetLinearVelocity();
